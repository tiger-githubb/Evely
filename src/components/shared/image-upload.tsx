...
export function ImageUpload({ value, onChange, className }: ImageUploadProps) {
  const t = useTranslations("Common.Logs"); // Translation hook
  ...
  const onDrop = useCallback(
    (acceptedFiles: File[], rejectedFiles: FileRejection[]) => {
      if (rejectedFiles.length > 0) {
        toast.error(t("fileUnsupported")); // Translated log
        return;
      }
      ...
      {imageUrl ? (
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-muted-foreground">
          ...
          <p className="text-center text-sm">
            {isDragging ? t("dragDropActive") : t("dragDropInactive")} {/* Translated logs */}
          </p>
        </div>
      )}
    </div>
